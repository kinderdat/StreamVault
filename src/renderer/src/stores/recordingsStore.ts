...
export const useRecordingsStore = create<RecordingsState>((set, get) => ({
  ...
  async stop(id: number) {
    ...
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, status: 'processing' as const } : r)),
      activeIds: new Set([...state.activeIds].filter((x) => x !== id)),
    }))
  ...
  markFailed(id: number) {
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, status: 'failed' as const } : r)),
      activeIds: new Set([...state.activeIds].filter((x) => x !== id)),
    }))
  },
  ...
  updateSnapshot(id: number, snapshotPath: string) {
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, thumbnail_path: snapshotPath } : r)),
    }))
  },

  updateMeta(id: number, meta: Partial<Recording>) {
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, ...meta } : r)),
    }))
  },

  updateSize(id: number, fileSize: number) {
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, file_size_bytes: fileSize } : r)),
    }))
  },
}))
